...
- Reusable app-agnostic Flutter infrastructure belongs in `packages/flutter_foundation` in Flexify rather than being copied between apps.
- Shared CI behavior belongs in reusable workflows hosted by Flexify; caller repositories should keep only app-specific orchestration and parameters.
- When a generic fix is made in one app, check whether the same failure pattern exists in sibling apps before considering the task complete.
